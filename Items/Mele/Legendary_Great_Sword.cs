...
namespace RemnantOfTheAncientsMod.Items.Mele
{
    public class Legendary_Great_Sword : ModItem
    {
        ...
        public override bool CanUseItem(Player player)
        {
            ...
            {
                var p = Projectile.NewProjectile(Projectile.GetSource_None(), player.position, new Vector2(1, 1), Item.shoot, Item.damage, Item.knockBack, Main.myPlayer, 2);
                comboExpireTimer = 0;  
            }
            return true;
        }
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.altFunctionUse != 2)
            {
                //Projectile.NewProjectile(Projectile.GetSource_None(), player.position, new Vector2(1, 1), Item.shoot, Item.damage, Item.knockBack, Main.myPlayer, 0);
                comboExpireTimer = 0;
                return true;
            }
