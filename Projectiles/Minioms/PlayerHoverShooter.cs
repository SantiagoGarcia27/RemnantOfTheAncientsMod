...
namespace RemnantOfTheAncientsMod.Projectiles.Minioms
{
	public abstract class PlayerHoverShooter : Minion
	{
		...
		public override void Behavior()
		{
			...
			for (int k = 0; k < 200; k++)
                    {
                        Player npc = Main.player[Projectile.owner];//Main.npc[k]
			...
			else
			{
				...
				Projectile.netUpdate = true;
				int num = 1;
				for (int k = 0; k < Projectile.whoAmI; k++) if (Main.projectile[k].active && Main.projectile[k].owner == Projectile.owner && Main.projectile[k].type == Projectile.type) num++;
		...
		public void shootprojectile(Vector2 targetPos)
        {
