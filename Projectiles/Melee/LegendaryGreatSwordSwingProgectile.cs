...
namespace RemnantOfTheAncientsMod.Projectiles.Melee
{
    ...
    public class LegendaryGreatSwordSwingProgectile : ModProjectile
	{
        ...
        private float SWINGRANGE = 1.67f * (float)Math.PI; // The angle a swing attack covers (300 deg)
        private float FIRSTHALFSWING = 0.45f; // How much of the swing happens before it reaches the target angle (in relation to swingRange)
        private float SPINRANGE = 3.5f * (float)Math.PI; // The angle a spin attack covers (630 degrees)
        private float WINDUP = 0.15f; // How far back the player's hand goes when winding their attack (in relation to swingRange)
        private float UNWIND = 0.4f; // When should the sword start disappearing
        private float SPINTIME = 4.5f; // How much longer a spin is than a swing
        ...
        private ref float Scaling => ref Projectile.ai[0];
        ...
        public override string Texture => "RemnantOfTheAncientsMod/Items/Mele/Legendary_Great_Sword"; // Use texture of item as projectile texture
        ...
        public override void SetDefaults()
        {
            ...
            Projectile.height = 78; // Hitbox height of projectile
        ...
        public void SetSwordPosition()
        {
            ...
            Scaling = Scaling < 1 ? 1 : Scaling < 2? Scaling : 2;
            Projectile.scale = Scaling * Size * 1.2f * Owner.GetAdjustedItemScale(Owner.HeldItem); // Slightly scale up the projectile and also take into account melee size modifiers
        ...
        private void PrepareStrike()
        {
            ...
            Size = MathHelper.SmoothStep(Scaling, 1, Timer / prepTime); // Make sword slowly increase in size as we prepare to strike until it reaches max
        ...
        private void ExecuteStrike()
        {
            ...
            {
                Progress = MathHelper.SmoothStep(0, SPINRANGE, (1f - UNWIND / 2) * Timer / (execTime * (SPINTIME * 0.5f)));

                if (Timer == (int)(execTime * (SPINTIME * 0.5f) * 3 / 4))
                {            
                    SoundEngine.PlaySound(SoundID.Item1); // Play sword sound again
                    Projectile.ResetLocalNPCHitImmunity(); // Reset the local npc hit immunity for second half of spin
                }

                if (Timer >= execTime * (SPINTIME * 0.5f))
                {
                    CurrentStage = AttackStage.Unwind;
                }
            }
        ...
        private void UnwindStrike()
        {
            ...
            {
                Progress = MathHelper.SmoothStep(0, SPINRANGE, (1f - UNWIND / 2) + UNWIND / 2 * Timer / (hideTime * (SPINTIME * 0.5f) / 2));
                Size = 1f - MathHelper.SmoothStep(0, 1, Timer / (hideTime * (SPINTIME * 0.5f) / 2));

                if (Timer >= hideTime * (SPINTIME * 0.5f) / 2)
                {
                    Projectile.Kill();
                }
            }
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                position = target.Center - new Vector2(Main.rand.NextFloat(401) * target.direction, 600f);
                position.Y -= 100 * i;
                ...
                var p = Projectile.NewProjectile(Projectile.GetSource_None(), position, heading, ProjectileID.BoulderStaffOfEarth, hit.Damage * 2, hit.Knockback, Owner.whoAmI, 0f, ceilingLimit);
            }
        }
    }
}