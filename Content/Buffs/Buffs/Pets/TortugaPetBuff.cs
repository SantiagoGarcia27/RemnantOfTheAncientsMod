...
namespace RemnantOfTheAncientsMod.Content.Buffs.Buffs.Pets
{
	public class TortugaPetBuff : ModBuff
	{
		...
		{ // This method gets called every frame your buff is active on your player.
			...
			int projType = ModContent.ProjectileType<TortugaPetProjectile>();

			// If the player is local, and there hasn't been a pet projectile spawned yet - spawn it.
			if (player.whoAmI == Main.myPlayer && player.ownedProjectileCounts[projType] <= 0)
			{
				Projectile.NewProjectile(player.GetSource_Buff(buffIndex), player.Center, Vector2.Zero, ModContent.ProjectileType<TortugaPetProjectile>(), 0, 0f, player.whoAmI);
			}
		}
	}
}